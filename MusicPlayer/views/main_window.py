...
class MainWindow(QWidget):
    def __init__(self, settings_mgr: SettingsManager, theme_mgr: ThemeManager):
        ...
        self.player.peak_changed.connect(self.peak_window.add_peak)
        ...
        self.peak_analyzer.finished.connect(self._on_peaks_ready)

        # 连接分析请求（歌曲添加时触发）
        self.playlist_win.analyze_requested.connect(self._request_peak_analysis)

        self.show()

    def _request_peak_analysis(self, file_path):
        # 启动后台峰值分析
        self.peak_analyzer.analyze(file_path)

    def _on_peaks_ready(self, file_path, peaks):
        # 将峰值存入对应 SongItem
        for song in self.playlist_win.playlist_model.songs:
            if song.file_path == file_path:
                song.peaks_normal = peaks
                break
