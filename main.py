...
def dump(oriPath, tarPath):
    ...
    print("处理完成，写入中...")
    m.write(result)
    audio_path = os.path.join(os.path.split(tarPath)[0], file_name)
    if image_data:
        try:
            ...
            if audio:
                if audio.__class__.__name__ == "FLAC":
                    ...
                    audio.save()
                elif audio.__class__.__name__ == "MP3":
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in list:
            if i.split(".")[-1]=="ncm":
                name = i.split("-")[-1].strip().replace(".ncm", "")
                if not os.path.isfile(os.path.join(tarPath, name+".flac")) and not os.path.isfile(os.path.join(tarPath, name+".mp3")):
                    temp.append(i)
        list = temp
        print(list)
        num = len(list)
        ...
        pass
        print("Usage: python ncmdump.py \"File Name\"",e)